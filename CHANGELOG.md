...
- **`rustloc diff` revspec resolution delegated to gix.** `rustloc diff
  v1.0.0..v2.0.0` (and any other tag-, branch-, or `HEAD~N`-style spec
  `git rev-parse` accepts) now works. Previously the CLI did its own
  naive `..` splitting and called `find_commit` directly on the resolved
  oid, which failed on annotated tags with "expected commit, got tag".
  The CLI now hands the revspec verbatim to the library, which calls
  `gix::Repository::rev_parse` and peels tag objects to commits via
  `peel_to_commit`. Range (`a..b`) and merge-base (`a...b`) syntax are
  parsed by gix natively. Two-arg form (`rustloc diff main feature`)
  still works — it's joined as `main..feature` before resolution.
