...
- `cargo install rustloc` failed to compile against `standout-dispatch`
  ≥ 7.6.2 because the `RunResult` match in `main.rs` was non-exhaustive
  and `RunResult` is `#[non_exhaustive]`. Local builds, CI, and
...
- **Release pipeline migrated to canonical reusable workflow at
  `arthur-debert/release/.github/workflows/rust-cli.yml@v1`.** rustloc's
  `.github/workflows/release.yml` is now a thin caller. Fourth consumer
  of the new pipeline (after dodot v2.0.0, padz v1.8.2, simple-gal v0.20.4 —
  all verified end-to-end). Bug fixes propagate via a single bump of the
  action's `@v1` ref instead of hand-edits across rust-CLIs.
- **Tarball naming + layout changed to canonical.** Per-target archives
  use full Rust target triples (`rustloc-x86_64-unknown-linux-gnu.tar.gz`)
  and contain a subdirectory matching the archive name. Direct downloads
  need to descend one directory level into the tarball.
...
- **Changed**:
  - **Releases now run end-to-end in CI via `scripts/release`.** Triggering a release with `scripts/release <version|major|minor|patch>` queues a `workflow_dispatch` run that performs the version bump (workspace.package + intra-workspace dep pin), `[Unreleased]` section roll, commit, tag, GitHub Release, multi-platform build (mac arm64+x86_64 signed+notarized, linux x86_64+arm64), `.deb` attach, crates.io publish, and Homebrew formula push to `arthur-debert/homebrew-tools` — all in CI. Replaces the previous local `cargo release` + tag-push trigger model.
